...
// ---- shared option lists + default config (module-level) ----
const V5_LESSON_OPTS = (()=>{
  const out=[];
  (window.PPCT||[]).forEach(w=>{ w.entries.forEach(e=>{ if(/^Đọc/.test(e.name)) out.push(`Tuần ${w.tuan} · ${e.name.replace(/^Đọc:\s*/,'')}`); }); });
  return out.slice(0,16);
})();
const V5_CHAPTER_OPTS = ['Cả chương 1 · Em lớn lên từng ngày (Tuần 1–4)','Giữa học kì 1 · Tuần 1–8','Cuối học kì 1 · Tuần 1–16'];
const v5DefaultCfg = () => ({
...
const v5DefaultState = id => ({ c: v5DefaultCfg(), lesson: V5_LESSON_OPTS[0]||'Tuần 1 · Tôi là học sinh lớp 2', chapter: V5_CHAPTER_OPTS[0] });

// ---- build a result-table row from item + its config ----
function v5ScopeTopic(state, isExam){
  const s = (isExam ? state.chapter : state.lesson) || '';
  const parts = s.split('·');
  return (parts[1]||parts[0]||'').trim();
}
function v5ResultRow(item, state){
  const c = state.c; const isExam = item.id.startsWith('bthi');
  const topic = v5ScopeTopic(state, isExam);
  const title = `${item.name} — ${topic}`;
  let detail='', count='1 nội dung';
  switch(item.id){
    case 'baidoc':     detail = `${c.dodai} · ${c.hinhanh?'có hình minh hoạ':'không hình'}`; break;
    case 'sachnoi':    detail = `${c.giong} · tốc độ ${c.tocdo}`; break;
    case 'video':      detail = `${c.thoiluong} · ${c.phongcach}`; break;
    case 'bt-chung':   detail = `${c.chon+c.sapxep+c.noi} câu · ${c.tg} phút · ${c.mucdo}`; break;
    case 'bt-rieng':   detail = `3 nhóm · 22 câu${c.sothich?' · cá nhân hoá':''}`; count='3 phiên bản'; break;
    case 'bthi-chung': detail = `${c.tn+c.tl} câu · ${c.tgthi} phút · ${c.thang}`; break;
    case 'bthi-rieng': detail = `${c.soma} mã đề · ${c.tgthi} phút${c.phanhoa?' · phân hoá':''}`; count=`${c.soma} mã đề`; break;
  }
  return { item, title, detail, count };
}
...
// ---- collapsible config card (one per selected item) — controlled ----
function V5ConfigCard({item, open, onToggle, onRemove, state, onChange}){
  const m = item;
  const c = state.c;
  const set = patch => onChange({...state, c:{...state.c, ...patch}});
  ...
  const opts = isExam ? V5_CHAPTER_OPTS : V5_LESSON_OPTS;
  const lessVal = isExam ? state.chapter : state.lesson;
  const lessSet = v => onChange({...state, [isExam?'chapter':'lesson']: v});
...
Object.assign(window, { V5_ROW1, V5_ROW2, V5_ALL, v5meta, v5ItemConfig, V5ConfigCard, v5DefaultState, v5ResultRow });
