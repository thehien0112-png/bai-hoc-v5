...
const summaryDesc = {
  setup:'Nội dung, mức độ và số lượng bài',
  baidoc:'Mỗi bài một bài đọc do AI tạo',
  sachnoi:'Mỗi bài một bản thu âm thanh',
  video:'Mỗi bài một video bài giảng',
  baitap:'Mỗi bài 10 câu hỏi luyện tập',
};
...
function App(){
  ...
  const resSummary = {
    baidoc:`${totalBai} bài đọc`,
    sachnoi:`${totalBai} sách nói · giọng ${cfg.voice.split(' (')[0]}`,
    video:`${totalBai} video · ${cfg.vstyle}`,
  ...
  return (
    <div className="app">
      ...
      <div className="page">
        ...
        <h1 className="page-title" style={{marginBottom:18}}>Tạo nội dung bằng AI <span className="v2-tag">V2</span></h1>
        ...
        <div className="acc">
          <Sec id="setup" index={1} title="Nguồn & thiết lập" open={open==='setup'}
            ...
            <p className="sec-sub">Nhập nội dung bài học (hoặc tải file), chọn số lượng bài theo mức độ. Mỗi bài gồm đủ Bài đọc, Sách nói, Video và 1 bài tập 10 câu.</p>
            ...
            <div className="qty-box">
              ...
              <p className="lvl-help">Chọn số bài muốn tạo ở mỗi mức độ. Mỗi bài gồm <b>10 câu hỏi</b> với tỉ lệ độ khó cố định, kèm Bài đọc / Sách nói / Video tương ứng.</p>
            ...
            <div className="acc-actions">
              <span className="foot-info">Sẽ tạo <b>{totalBai}</b> bài × <b>{orderedResults.length}</b> loại nội dung</span>
          ...
          {phase==='ready' && orderedResults.map((t,i)=>{
            ...
            const body = ()=>{
              if(st==='generating'){
                if(t.id==='sachnoi') return <GenericGenerating label="Sách nói" pct={audioPct} sub={`${totalBai} bản · giọng ${cfg.voice}`} onCancel={()=>cancel('sachnoi',audioTimer)} accent={{c:'#7c3aed',bd:'#e2d5f5',bg:'linear-gradient(180deg,#fcfaff,#f6f0fe)',ring:'#e9ddf8'}}/>;
                if(t.id==='video') return <GenericGenerating label="Video" pct={videoPct} sub={`${totalBai} video · ${cfg.vstyle}`} onCancel={()=>cancel('video',videoTimer)} accent={{c:'#ea580c',bd:'#fad9bf',bg:'linear-gradient(180deg,#fffaf5,#fff1e6)',ring:'#fbd9bd'}}/>;
                return <div className="skel-note"><span className="spin"/>Đang tạo {totalBai} {t.unit}…</div>;
              }
              if(st==='cancelled'){
              ...
              // done → list theo bài
              if(t.id==='baidoc') return <BaiList bais={bais} color={t.color} unitLabel="Bài đọc"
                sub={b=>'~86 từ · '+b.level} renderBody={()=> <ReadingBody/>}/>;
              if(t.id==='sachnoi') return <BaiList bais={bais} color={t.color} unitLabel="Sách nói"
                sub={b=>'2:14 · giọng '+cfg.voice.split(' (')[0]} renderBody={()=> <AudioBody/>}/>;
              if(t.id==='video') return <BaiList bais={bais} color={t.color} unitLabel="Video"
                sub={b=>'1:48 · '+cfg.vstyle} renderBody={()=> <VideoBody style={cfg.vstyle}/>}/>;
              return (
                <div>
          ...
          <div className="save-bar">
            <span className="save-info">
              {(audioBusy||videoBusy)
                ...
                : <>Đã tạo <b>{totalBai}</b> bài cho <b>“{LESSON.title}”</b></>}
            </span>
